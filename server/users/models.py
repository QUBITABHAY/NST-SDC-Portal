...
class User(AbstractUser):
    ...
    student_id = models.CharField(
        max_length=50, blank=True, null=True, help_text="Student ID / Roll Number"
    )

    batch_year = models.IntegerField(
        blank=True, null=True, help_text="Graduation Year (e.g., 2025)"
    )

    # Dev Club specific fields
    points = models.IntegerField(
        default=0, help_text="Gamification points for leaderboard"
    )

    github_username = models.CharField(
        max_length=100, blank=True, null=True, help_text="GitHub username"
    )
    ...
    portfolio_url = models.URLField(
        blank=True, null=True, help_text="Personal portfolio website"
    )
    ...
    provider_id = models.CharField(
        max_length=255, blank=True, null=True, help_text="Unique ID from OAuth provider"
    )

    github_id = models.CharField(
        max_length=255, blank=True, null=True, help_text="GitHub OAuth user ID"
    )

    # Role flags
    is_member = models.BooleanField(
        default=False, help_text="Whether user is a club member"
    )

    is_club_admin = models.BooleanField(
        default=False, help_text="Whether user is a club administrator"
    )
